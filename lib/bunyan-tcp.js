...
MessageBuffer.prototype.drain = function(cb) {
  ...
  this.buffer = [];
  var startIdx = this.messagesAdded % old_buffer.length;
  for (var i=startIdx; i<(old_buffer.length + startIdx); i++) {
    cb(old_buffer[i % this.messageMax]);
  }
  this.messagesDropped = 0;
  delete old_buffer;
}
...
BunyanTcpStream.prototype.flush = function() {
  this.socket.flush();
};
